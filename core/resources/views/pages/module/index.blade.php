...
<div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-body">
                @include('pages.module.search')
                <table id="dataTableModule" class="table table-striped dt-responsive  nowrap w-100">
                    <thead>
                        <tr>
                            <th>
...
<script>
    var table;
        $(document).ready(function() {
            ...
            var table = $('#dataTableModule').DataTable({
                // "dom": 'Bfrtip',
                // "dom": 'lfrtip',
            ...
            $('#dataForm').on('submit', function(event) {
                ...
                if (id) {
                    var method = 'update';
                    var url = "{{ route('admin.modules.update', ':id') }}";
                    url = url.replace(':id', id);
                } else {
                    var method = 'store';
                    var url = "{{ route('admin.modules.store') }}";
                }
            ...
            $(document).on("click", ".editBtn", function() {
                var id = $(this).data("id");
                var url = "{{ route('admin.modules.edit', ':id') }}";
                url = url.replace(":id", id);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $("#dataForm #name").val(data.module.name);
                        $("#dataForm #link").val(data.module.link);
                        $("#dataForm #icon").val(data.module.icon);
                        $("#dataForm #sequence").val(data.module.sequence);
                        $("#dataForm #parent_id").val(data.module.parent_id);
                        $("#dataForm #moduleStatus").val(data.module.status);
                        $("#dataForm #updateId").val(data.module.id);
            ...
            $(document).on("click", ".deleteBtn", function() {
                ...
                var id = $(this).data("id");
                var url = "{{ route('admin.modules.destroy', ':id') }}";
                url = url.replace(":id", id);
            ...
            $(document).on("click", "#bulk_action_delete", function(e) {
                ...
                if (id.length === 0) {
                    ...
                    var url = "{{ route('admin.modules.bulk_delete') }}";
        ...
        $('#name').on('keyup', function() {
            urlGenerator(this.value, output_id = 'link');
        });
        ...
        $(document).on("change", ".change-status", function() {
            ...
            id = $(this).data("id");
            $.ajax({
                url: "{{ route('admin.modules.change_status') }}",
                type: "POST",
                data: {
                    id: id,
                    status: status
        ...
        function load() {
            $.ajax({
                url: `{{ route('admin.modules.load') }}`,
                type: "GET",
                success: function(data) {
                    console.log(data);
                    if (data) {
                        $('#dataForm #parent_id').append(`<option value="${data.id}">${data.name}</option>`);
                    }
                },
                error: function(xhr, ajaxOptions, thrownError) {
                    console.log(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
