...
<div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-body">
                <form id="languageSearch" method="POST">
                    <div class="row">
                        @include('pages.language.search')
                        @include('partials.search_button')
                    </div>
                </form>
                <table id="dataTableLanguage" class="table table-striped dt-responsive  nowrap w-100">
                    <thead>
                        <tr>
                            <th>
...
<script>
    var table;
        $(document).ready(function() {
            ...
            var table = $('#dataTableLanguage').DataTable({
                // "dom": 'Bfrtip',
            ...
            $('#dataForm').on('submit', function(event) {
                ...
                if (id) {
                    var method = 'update';
                    var url = "{{ route('admin.languages.update', ':id') }}";
                    url = url.replace(':id', id);
                } else {
                    var method = 'store';
                    var url = "{{ route('admin.languages.store') }}";
                }
            ...
            $(document).on("click", ".editBtn", function() {
                var id = $(this).data("id");
                var url = "{{ route('admin.languages.edit', ':id') }}";
                url = url.replace(":id", id);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $("#dataForm #name").val(data.language.name);
                        $("#dataForm #code").val(data.language.code);
                        $("#dataForm #updateId").val(data.language.id);
            ...
            $(document).on("click", "#bulk_action_delete", function(e) {
                ...
                if (id.length === 0) {
                    ...
                    var url = "{{ route('admin.languages.bulk_delete') }}";
