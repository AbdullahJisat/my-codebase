...
use App\Rules\BdPhoneVerificationRule;
...
class UserController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        foreach ($list as $value) {
            ...
            $action .= "<a href='javascript:void(0);' class='text-info showBtn' data-id='{$value->id}'><i class='mdi mdi-eye font-size-18'></i></a>";
            ...
            $row[]  = "<a href=" . route('admin.users.show', $value->id) . ">$value->name</a>";
            $row[]  = $value->email;
            $row[]  = $value->username;
            $row[]  = $value->mobile;
            $row[]  = $value->gender;
            $row[]  = "<span style='text-align:center;' class='form-check form-switch form-switch-md'><label class='form-check-label' for='$value->username'><input value='$status' class='change-status form-check-input' name='status' data-id='$value->id' type='checkbox' id='$value->username' $checked></label></span>";
        ...
        return [
            ...
            "recordsFiltered" => \App\Models\User::count(),
            "data" => $data
        ];
    }
    ...
    public function getList($_lengthValue, $_startValue, $_searchString)
    {
        ...
        if (!empty($_searchString)) {
            $query->where('users.name', 'like', "%{$_searchString}%")
                ->orWhere('users.email', 'like', "%{$_searchString}%")
                ->orWhere('users.username', 'like', "%{$_searchString}%")
                ->orWhere('users.mobile', 'like', "%{$_searchString}%");
        }
    ...
    {
        $query = DB::table('users')->get()->count();
        return $query;
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $validator = Validator::make($request->all(), [
                ...
                'mobile' => new BdPhoneVerificationRule,
            ], [
            ...
            $this->userContract->create($request->merge(['password' => Hash::make($request->password)])->all());
            return response()->json([
                'status' => 'success',
